...
from userbot import CMD_HELP, bot, BIO_MESSAGE, BIO_MSG, DetronBot_NAME
...
FONT_FILE_TO_USE = "userbot/DetronBot_main/extra/DejaVuSansMono.ttf"
...
async def rk():
    while True:    	        
        ...
        nnmel = f"{HM}"
        #await event.reply(f"**{DetronBot_NAME}**: `successfully set last name to {HM} {TZZ}\n Sleeping 60s........`")        
        try:        	
...
async def rk2():
    while True:        
        ...
        bio = f" {BIO_MMSG}| {HM}"
        #await event.reply(f"**{DetronBot_NAME}**: `successfully set auto bio with {BIO_MMSG} with {HM} {TZZ}\n Sleeping 60s.......`")
...
async def rk3():
    ...
    while True:
        ...
        #await event.reply(f"**{DetronBot_NAME}**: `successfully set profile picture \nSleeping 60s.......`")
...
@DetronBot05(outgoing=True, pattern="^!autoname")
async def _(event):
    while True:    	        
        NT = datetime.now(pytz.timezone(TZZ))
        HM = NT.strftime("%I:%M%p")
...
@DetronBot05(outgoing=True, pattern="^!autobio$")
async def _(event):  
    while True:        
        DMY = time.strftime("%Y:%m:%d ")
        NT = datetime.now(pytz.timezone(TZZ))
...
@DetronBot05(outgoing=True, pattern="^!autopic$")
async def autopic(event):
    await event.reply(f"**{DetronBot_NAME}**: `Auto pic Activated`")
    downloaded_file_name = "userbot/original_pic.png"
    downloader = SmartDL(AUTO_PP, downloaded_file_name, progress_bar=False)
    downloader.start(blocking=False)
    photo = "userbot/photo_pfp.png"
...
@DetronBot05(outgoing=True, pattern="^!runclock$")
async def _(event):
    if event.fwd_from:
        return
    while True:
...
@DetronBot05(outgoing=True, pattern="^!autopic2$")
async def _(event):
 FONT_FILE_TO_USE = "userbot/DetronBot_main/extra/JollyBold-ZGW3.ttf"
 downloaded_file_name = "userbot/original_pic.png"
 downloader = SmartDL(AUTO_PP, downloaded_file_name, progress_bar=False)
 downloader.start(blocking=False)
 photo = "userbot/photo_pfp.png"
...
\n\n**For stop this restart DetronBot**\
