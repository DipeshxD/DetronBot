...
from userbot import (HEROKU_APP_NAME, HEROKU_API_KEY, CMD_HELP, BOTLOG, BOTLOG_CHATID, DetronBot_NAME)
from userbot.events import DetronBot05, rekcah05 as admin_cmd
...
@DetronBot05(outgoing=True, pattern=r"^!(set|get|del) var(?: |$)(.*)(?: |$)")
...
@DetronBot.on(admin_cmd(pattern=f"(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(var):
  try:
     Heroku = heroku3.from_key(HEROKU_API_KEY)                         
     app = Heroku.app(HEROKU_APP_NAME)
  	...
  	return await var.reply(" `Please make sure your Heroku API Key, Your App name are configured correctly in the heroku var` please check https://telegra.ph/RkPavi-06-09-6")
  if not FULL_SUDO:
      await var.reply(f"`{DetronBot_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**") 
  else:
    """
...
@DetronBot05(outgoing=True, pattern="^\!usage$")
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot.on(admin_cmd(pattern=f"usage$", allow_sudo=True))
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot05(outgoing=True, pattern="^\!shutdown$")
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot.on(admin_cmd(pattern=f"shutdown$", allow_sudo=True))
async def _(dyno):        
  try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
  	       ...
  	       return await dyno.reply(" `Please make sure your Heroku API Key, Your App name are configured correctly in the heroku var` please check https://telegra.ph/RkPavi-06-09-6")
  if not FULL_SUDO:
      await dyno.reply(f"`{DetronBot_NNAME}:` **Sorry , Normal Sudo cant acess this comand,  active advance sudo by set  FULL_SUDO as true in heroku var**") 
  else:  	  
        app.scale_formation_process("worker", 0)
        text = f"`Turning Off Dynos` "
        sleep = 1
...
@DetronBot05(outgoing=True, pattern="^\!logs$")
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot.on(admin_cmd(pattern=f"logs$", allow_sudo=True))
async def _(dyno):        
  try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot05(outgoing=True, pattern="^\!restart$")
async def _(dyno):        
        try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
@DetronBot.on(admin_cmd(pattern=f"restart$", allow_sudo=True))
async def _(dyno):        
  try:
             Heroku = heroku3.from_key(HEROKU_API_KEY)                         
             app = Heroku.app(HEROKU_APP_NAME)
...
CMD_HELP.update({
    ...
    "\nUsage: Restart Dynos"
    "\n\n`!shutdown <VAR>`"
    "\nUsage: off dynos, It will shutdown your DetronBot"
