...
@DetronBot05(outgoing=True, pattern="^!saveblacklist ((.|\n)*)")
...
@DetronBot.on(rekcah05(pattern=f"saveblacklist ((.|\n)*)", allow_sudo=True))
async def on_add_black_list(event):
    text = event.pattern_match.group(1)
    to_blacklist = list(set(trigger.strip() for trigger in text.split("\n") if trigger.strip()))
    for trigger in to_blacklist:
...
@DetronBot05(outgoing=True, pattern="^!checkblacklist(?: |$)(.*)")
...
@DetronBot.on(rekcah05(pattern=f"checkblacklist$", allow_sudo=True))
async def on_view_blacklist(listbl):
    all_blacklisted = sql.get_chat_blacklist(listbl.chat_id)
    OUT_STR = "Blacklists in the Current Chat:\n"
    if len(all_blacklisted) > 0:
...
@DetronBot(outgoing=True, pattern="^!clearblacklist ((.|\n)*)")       
...
@DetronBot.on(rekcah05(pattern=f"clearblacklist ((.|\n)*)", allow_sudo=True))
async def on_delete_blacklist(event):
    text = event.pattern_match.group(1)
    to_unblacklist = list(set(trigger.strip() for trigger in text.split("\n") if trigger.strip()))
    successful = 0
