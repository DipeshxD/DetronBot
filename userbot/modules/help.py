...
from userbot import bot as @DetronBot, CMD_HELP
import os
...
if not IN:
 ...
 async def help(event):
    ...
    if args:
        ...
        await event.edit(" For Support, Report bugs & help @@DetronBot_suPPort ")
...
@DetronBot.on(rekcah05(pattern=f"help(?: |$)(.*)", allow_sudo=True))
async def help(event):
    args = event.pattern_match.group(1).lower()
    if args:
        if args in CMD_HELP:
        ...
        await event.reply(" For Support, Report bugs & help @DetronBot_SuPPort ")
        string = (f"`Use .help <module_name>`\n\n**Currently Loaded [{len(CMD_HELP)}] Modules **\n")
        for i in CMD_HELP:
            string += "`" + str(i)
            string += "`\n"
...
CMD_HELP.update({
    "sudo":
    "if you active sudo , sudo users can controll your @DetronBot like you controll groupmanaging bots\
    ...
    \n video example :- https://t.me/DetronBot/116 \
