...
from userbot.events import DetronBot05, rekcah05
from userbot import client as DetronBot
FULL_SUDO = os.environ.get("FULL_SUDO", None)


@DetronBot.on(rekcah05(pattern=f"(?:scam|ssc)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:scam|ssc)\s(.*)")
async def scam(e):
    if not e.is_private:
...
@DetronBot.on(rekcah05(pattern=f"(?:rspam|rsp)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:rspam|rsp)\s(.*)")
async def repeat(e):
...
@DetronBot.on(rekcah05(pattern=f"(?:cspam|csp)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:cspam|csp)\s(.*)")
async def tmeme(e):
  sender = await e.get_sender() ; me = await e.client.get_me()
  if not sender.id == me.id and not FULL_SUDO:
       return await e.reply("`Sorry normal sudo users cant access this command..`")
...
@DetronBot.on(rekcah05(pattern=f"(?:wspam|wsp)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:wspam|wsp)\s(.*)")
async def tmeme(e):
  sender = await e.get_sender() ; me = await e.client.get_me()
  if not sender.id == me.id and not FULL_SUDO:
       return await e.reply("`Sorry normal sudo users cant access this command..`")
...
@DetronBot.on(rekcah05(pattern=f"(?:dmspam|dmsp)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:dmspam|dmsp)\s(.*)")
...
@DetronBot.on(rekcah05(pattern=f"(?:delayspam|dsp)\s(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern=r"^\!(?:delayspam|dsp)\s(.*)")
async def spammer(e):
  sender = await e.get_sender() ; me = await e.client.get_me()
  if not sender.id == me.id and not FULL_SUDO:
       return await e.reply("`Sorry normal sudo users cant access this command..`")
...
\n More information click [here](https://t.me/DetronBot05/157)\
