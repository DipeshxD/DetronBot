...
@DetronBot05(outgoing=True, pattern="^!decompile(?: |$)(.*)")
async def _(event):  
    textt = "**Terminal Status**"
    ...
    if stdout or stderr:
       ...
       output.write(text)
       output.close()
       await event.client.send_file(event.chat_id, "decompiled.txt", reply_to=event.id, caption=f"`{DetronBot_NNAME}:` **Decompiled Details**")
...
@DetronBot05(outgoing=True, pattern="^!bindapk(?: |$)(.*)")
async def _(event):
    await event.edit(f"{textt}\n\nAnalyzing Datas......")
    message = (str(event.chat_id) + ':' + str(event.message.id))
    input_str = event.pattern_match.group(1)
...
@DetronBot05(outgoing=True, pattern="^!payloadapk(?: |$)(.*)")
async def _(event):
     await event.edit(f"{textt}\n\nAnalyzing Datas......")
     input_str = event.pattern_match.group(1)
     if not input_str:
