#telegram channel @DetronBot2support
...
from userbot.DetronBot_main.heroku_var import config as Config
from userbot.DetronBot_main.heroku_var import config as Var
from userbot.DetronBot_main.heroku_var import config as var
...
import os
from userbot.events import DetronBot05, bot, rekcah05
from userbot.util import admin_cmd
from userbot import tebot as tgbot
from userbot import bot as borg
DetronBot = client = bot
from telethon import events
...
if IN:
  @DetronBot05(outgoing=True, pattern="^!help(?: |$|\n)([\s\S]*)")
  async def ban(event):
    if not BT:
 ...
 async def handler(event):
  try:
    ...
    if et == "back":
        ...
        return await event.edit ("Support Group For Report bugs & help @DetronBot_Suppport", buttons=tbu, link_preview=False)   
    if et in CMD_HELP: 
...
if tebot:
 ...
 async def inline_handler(event):
  ...
  if query.startswith("helpme"):
      ...
      if sad3:
	       tbu += [[ Button.inline(f"{sad3}"  , f"{sad3}")]]   
      result = builder.article("Help menu", text = "Support Group For Report bugs & help @DetronBot2support", buttons=tbu, link_preview=False)      
...
#some codes taken form Cat bot Fixing them to DetronBot was a task #Shivam
