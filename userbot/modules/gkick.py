...
try:
   ...
   pass
@DetronBot.on(rekcah05(pattern=f"gkick(?: |$)(.*)", allow_sudo=True))
@DetronBot(outgoing=True, pattern="^!gkick(?: |$)(.*)")
async def gspide(rk): 
   ...
   me = await rk.client.get_me() ; await rkp.edit(f"`{DetronBot_NNAME}:` **Requesting  to gkick user!**") ; my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id) ; my_username = f"@{me.username}" if me.username else my_mention ; chat = await rk.get_chat() ; a = b = 0
   if rk.is_private:       
   ...
   if user:      
        if user.id == 1801571739:     
    	             return await rkp.edit(f"`{DetronBot_NNAME}:`**HEY THAT'S MY DEV**")
        try:
        ...
        for i in testrk:
            try:
                 ...
                 a += 1
                 await rkp.edit(f"`{DetronBot_NNAME}:` **Requesting  to gkicking user!\nGkicked {a} chats.....**")
   ...
   return await rkp.edit(f"`{DetronBot_NNAME}:` **GKicked [{user.first_name}](tg://user?id={user.id}) in {a} chat(s) **") 
