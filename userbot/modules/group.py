...
from userbot.events import DetronBot05
...
# =================== CONSTANT ===================
PP_TOO_SMOL = f"`{DetronBot_NNAME}:`**The image is too small**"
PP_ERROR = f"`{DetronBot_NNAME}:`**Failure while processing the image**"
NO_ADMIN = f"`{DetronBot_NNAME}:`**Sorry, I can't able to get admin rights here!**"
NO_PERM = f"`{DetronBot_NNAME}:`**I don't have sufficient permissions!**"
NO_SQL = f"`{DetronBot_NNAME}:`**Running on Non-SQL mode!**"
CHAT_PP_CHANGED = f"`{DetronBot_NNAME}:`**Chat Picture Changed**"
CHAT_PP_ERROR = f"`{DetronBot_NNAME}:`**Some issue with updating the pic,**" \
...
#filters logic
@DetronBot.on(events.NewMessage(incoming=True))
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!userid$")
...
@DetronBot.on(rekcah05(pattern=f"userid$", allow_sudo=True))
async def useridgetter(target):
    """ For .userid command, returns the ID of the target user. """
    message = await target.get_reply_message()
    if message:
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!link(?: |$)(.*)")
async def permalink(mention):
    ...
    if custom:
        ...
        await mention.edit(f"`{DetronBot_NNAME}`: [{tag}](tg://user?id={user.id})")


@DetronBot.on(rekcah05(pattern=f"link(?: |$)(.*)", allow_sudo=True))
async def permalink(mention):
    """ For .link command, generates a link to the user's PM with a custom text. """
    user, custom = await get_user_from_event(mention)
    if not user:
    ...
    if custom:
        ...
        tag = user.first_name.replace("\u2060",
                                      "") if user.first_name else user.username
        await mention.reply(f"`{DetronBot_NNAME}`: [{tag}](tg://user?id={user.id})")



@DetronBot(outgoing=True, disable_errors=True, pattern="^\!chatid$")
async def chatidgetter(chat):
    """ For .chatid, returns the ID of the chat you are in at that moment. """
    await chat.edit(f"`{DetronBot_NNAME}`: Chat ID: `" + str(chat.chat_id) + "`")


@DetronBot.on(rekcah05(pattern=f"chatid$", allow_sudo=True))
async def chatidgetter(chat):
    """ For .chatid, returns the ID of the chat you are in at that moment. """
    await chat.reply(f"`{DetronBot_NNAME}`: Chat ID: `" + str(chat.chat_id) + "`")




@DetronBot(outgoing=True, disable_errors=True, pattern=r"^\!log(?: |$)([\s\S]*)")
async def log(log_text):
    ...
    if BOTLOG:
        if log_text.reply_to_msg_id:
            ...
            await log_text.edit(f"`{DetronBot_NNAME}`: **What am I supposed to log?**")
            return
        await log_text.edit(f"`{DetronBot_NNAME}`: **Logged Successfully**")
    else:
        await log_text.edit(f"`{DetronBot_NNAME}`: **This feature requires Logging to be enabled!**")
    await sleep(2)
    await log_text.delete()

@DetronBot.on(rekcah05(pattern=f"log$", allow_sudo=True))
async def iqless(e):
    await e.reply(f"`{DetronBot_NNAME}`: **Privacy error! , Sorry sudo users dont have permission to access it!**")
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!kickme$")
async def kickme(leave):
    """ Basically it's .kickme command """
    await leave.edit(f"`{DetronBot_NNAME}`: **My master Didnt like this place......GoodBye!**")
    await leave.client.kick_participant(leave.chat_id, 'me')

@DetronBot.on(rekcah05(pattern=f"kickme$", allow_sudo=True))
async def iqless(e):
    await e.reply(f"`{DetronBot_NNAME}`: **Privacy error! , Sorry sudo users dont have permission to access it!**")
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!delusers(?: |$)(.*)", groups_only=True)
async def rm_deletedacc(show):
    """ For .delusers command, list all the ghost/deleted accounts in a chat. """
    if not show.is_group:
        await show.edit(f"`{DetronBot_NNAME}:` ** This is not a group.**")
        return
    con = show.pattern_match.group(1).lower()
    del_u = 0
    del_status = f"`{DetronBot_NNAME}:` **No deleted accounts found**"

    if con != "clean":
        await show.edit(f"`{DetronBot_NNAME}:` ** Searching for deleted accounts...**")
        async for user in show.client.iter_participants(show.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(1)
        if del_u > 0:
            del_status = f"`{DetronBot_NNAME}:` Found **{del_u}** deleted account(s) in this group,\
    ...
    if not admin and not creator:
        await show.edit(f"`{DetronBot_NNAME}:` **Sorry,  I can't able to get admin rights here**")
        return

    await show.edit(f"`{DetronBot_NNAME}:` ** Removing deleted accounts...**")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await show.edit(f"`{DetronBot_NNAME}:` **Sorry, I don't have ban rights in this group")
                return
    ...
    if del_u > 0:
        del_status = f"`{DetronBot_NNAME}`: Cleaned **{del_u}** deleted account(s)"
...
@DetronBot.on(rekcah05(pattern=f"delusers(?: |$)(.*)", allow_sudo=True))
async def rm_deletedacc(show):
    """ For .delusers command, list all the ghost/deleted accounts in a chat. """
    if not show.is_group:
        await show.reply(f"`{DetronBot_NNAME}:` ** This is not a group.**")
        return
    con = show.pattern_match.group(1).lower()
    del_u = 0
    del_status = f"`{DetronBot_NNAME}:` **No deleted accounts found**"
    ...
    if con != "clean":
        await show.reply(f"`{DetronBot_NNAME}:` ** Searching for deleted accounts...**")
        async for user in show.client.iter_participants(show.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(1)
    ...
    # Well
    if not admin and not creator:
        await show.reply(f"`{DetronBot_NNAME}:` **Sorry,  I can't able to get admin rights here**")
        return

    await show.reply(f"`{DetronBot_NNAME}:` ** Removing deleted accounts...**")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await show.reply(f"`{DetronBot_NNAME}:` **Sorry, I don't have ban rights in this group")
                return
            except UserAdminInvalidError:
                del_u -= 1
                del_a += 1
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!admins$", groups_only=True)
async def get_admin(show):
    ...
    try:
        ...
        await show.edit(
            f"`{DetronBot_NNAME}`: **Too many admins here. Uploading admin list as file**")
...
@DetronBot.on(rekcah05(pattern=f"admins$", allow_sudo=True))
async def get_admin(show):
    """ For .admins command, list all of the admins of the chat. """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!bots$", groups_only=True)
async def get_bots(show):
    ...
    try:
        ...
        await show.edit(
            f"`{DetronBot_NNAME}`: ** Too many bots here. Uploading bots list as file.**")
...
@DetronBot.on(rekcah05(pattern=f"bots$", allow_sudo=True))
async def get_bots(show):
    """ For .bots command, list all of the bots of the chat. """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!users ?(.*)", groups_only=True)
async def get_users(show):
    ...
    try:
        ...
        await show.edit(
            f"`{DetronBot_NNAME}`: ** This is a huge group. Uploading users lists as file.")
...
@DetronBot.on(rekcah05(pattern=f"users ?(.*)", allow_sudo=True))
async def get_users(show):
    """ For .users command, list all of the users in a chat. """
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if not user:
            await event.reply(f"`{DetronBot_NNAME}`: ** Pass the user's username, id or reply!**")
            return
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!savefilter2 (\w*)")
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            ...
            await new_handler.edit(
                f"`{DetronBot_NNAME}`: ** Saving media as reply to the filter requires the BOTLOG_CHATID to be set.**"
            )
            return
...
@DetronBot.on(rekcah05(pattern=f"savefilter2 (\w*)", allow_sudo=True))
async def add_new_filter(new_handler):
    """ For .filter command, allows adding new filters in a chat """
    try:
        from userbot.modules.sql_helper.filter_sql import add_filter
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!clearfilter2 (\w*)")
...
@DetronBot.on(rekcah05(pattern=f"clearfilter2 ?(.*)", allow_sudo=True))
async def remove_a_filter(r_handler):
    """ For .stop command, allows you to remove a filter from a chat. """
    try:
        from userbot.modules.sql_helper.filter_sql import remove_filter
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!checkfilter2$")
async def filters_active(event):
    ...
    try:
        ...
        return
    transact = f"`{DetronBot_NNAME}`: ** There are no filters in this chat.**"
...
@DetronBot.on(rekcah05(pattern=f"checkfilter2$", allow_sudo=True))
async def filters_active(event):
    """ For .filters command, lists all of the active filters in a chat. """
    try:
        from userbot.modules.sql_helper.filter_sql import get_filters
...
@DetronBot(pattern="!chatinfo(?: |$)(.*)", outgoing=True)
...
@DetronBot.on(rekcah05(pattern=f"chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^!resetwarns(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
...
@DetronBot.on(rekcah05(pattern=f"resetwarns(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    reply_message = await event.get_reply_message()
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^!invite(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    to_add_users = event.pattern_match.group(1)
    if event.is_private:
        await event.edit(f"**{DetronBot_NNAME}:** invite  users to a chat, not to a Private Message")
    else:
        if not event.is_channel and event.is_group:
            ...
            await event.edit(f"**{DetronBot_NNAME}:** Invited Requesr sent Successfully")
        else:
            ...
            for user_id in to_add_users.split(" "):
                try:
                    ...
                    await event.reply(str(e))
            await event.edit(f"**{DetronBot_NNAME}:** Invited Successfully")

@DetronBot.on(rekcah05(pattern=f"invite(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    to_add_users = event.pattern_match.group(1)
    if event.is_private:
        await event.reply(f"**{DetronBot_NNAME}:** invite  users to a chat, not to a Private Message")
    else:
        if not event.is_channel and event.is_group:
            # https://lonamiwebs.github.io/Telethon/methods/messages/add_chat_user.html
            for user_id in to_add_users.split(" "):
                try:
                    ...
                    await event.reply(str(e))
            await event.reply(f"**{DetronBot_NNAME}:** Invite request sent telethon Successfully")
        else:
            # https://lonamiwebs.github.io/Telethon/methods/channels/invite_to_channel.html
            for user_id in to_add_users.split(" "):
                try:
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^!savefilter (.*)")
async def on_snip_save(event):
    ...
    if msg:
        ...
        await event.edit(f"`{DetronBot_NNAME}`: filter {name} saved successfully. Get it with {name}")
    else:
        await event.edit(f"`{DetronBot_NNAME}`: **Reply to a message with `!savefilter keyword` to save the filter**")


@DetronBot.on(rekcah05(pattern=f"savefilter (.*)", allow_sudo=True))
async def on_snip_save(event):
    name = event.pattern_match.group(1)
    msg = await event.get_reply_message()
    if msg:
        ...
        if msg.media:
            ...
            if media:
                ...
                snip['fr'] = media.file_reference
        add_filter(event.chat_id, name, snip['text'], snip['type'], snip.get('id'), snip.get('hash'), snip.get('fr'))
        await event.reply(f"`{DetronBot_NNAME}`: filter {name} saved successfully. Get it with {name}")
    else:
        await event.reply(f"`{DetronBot_NNAME}`: **Reply to a message with `.savefilter keyword` to save the filter**")


@DetronBot(outgoing=True, disable_errors=True, pattern="^\!checkfilter$")
async def on_snip_list(event):
    all_snips = get_all_rkfilters(event.chat_id)
    OUT_STR = f"`{DetronBot_NNAME}`: Available filters in the Current Chat:\n"
    if len(all_snips) > 0:
        for a_snip in all_snips:
            OUT_STR += f"~> {a_snip.keyword} \n"
    else:
        OUT_STR = f"`{DetronBot_NNAME}`: No filters. Start Saving using `!savefilter`"
    if len(OUT_STR) > 4096:
        with io.BytesIO(str.encode(OUT_STR)) as out_file:
            ...
            await bot.send_file(
                ...
                force_document=True,
                allow_cache=False,
                caption=f"`{DetronBot_NNAME}`: **Available filters in the Current Chat**",
                reply_to=event
            )
            await event.delete()
    else:
...
@DetronBot.on(rekcah05(pattern=f"checkfilter$", allow_sudo=True))
async def on_snip_list(event):
    all_snips = get_all_rkfilters(event.chat_id)
    OUT_STR = f"`{DetronBot_NNAME}`: Available filters in the Current Chat:\n"
    if len(all_snips) > 0:
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!clearfilter (\w*)")
async def on_snip_delete(event):
    name = event.pattern_match.group(1)
    remove_filter(event.chat_id, name)
    await event.edit(f"`{DetronBot_NNAME}`: filter {name} deleted successfully")

@DetronBot.on(rekcah05(pattern=f"clearfilter (.*)", allow_sudo=True))
async def on_snip_delete(event):
    name = event.pattern_match.group(1)
    remove_filter(event.chat_id, name)
    await event.edit(f"`{DetronBot_NNAME}`: filter {name} deleted successfully")
...
@DetronBot(outgoing=True, disable_errors=True, pattern="^\!clearallfilter$")
async def on_all_snip_delete(event):
    remove_all_rkfilters(event.chat_id)
    await event.edit(f"`{DetronBot_NNAME}`: filters **in current chat** deleted successfully")




@DetronBot.on(rekcah05(pattern=f"clearallfilter$", allow_sudo=True))
async def on_all_snip_delete(event):
    remove_all_rkfilters(event.chat_id)
    await event.reply(f"`{DetronBot_NNAME}`: filters **in current chat** deleted successfully")
