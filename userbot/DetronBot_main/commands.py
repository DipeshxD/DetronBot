...
from userbot.DetronBot_main.heroku_var import *
from userbot import *
from sys import *
        ...
        async def wrapper(check):
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    text = "**DetronBot ERROR REPORT**\n"
                    text += "Send this to @DetronBotsupport if you cant find issue\n"
                    ftext = "========== DISCLAIMER =========="
                    ...
                    ftext += result
                    file = open("DetronBot_error.log", "w+")
                    file.write(ftext)
                    file.close()
                    try:                 
                      await check.client.send_file(send_to, "DetronBot_error.log", caption=text)
                      remove("DetronBot_error.log")
                    except:
                      pass
...
borg = DetronBot = bot ; admin_cmd = rekcah05 ; command = zzaacckkyy ; register = DetronBot05 = DetronBots
...
data = json.load(open("userbot/DetronBot_main/extra/meaning.json")) 
def meaning(w): 
	w = w.lower() 
	if w in data: 
		return data[w] 
